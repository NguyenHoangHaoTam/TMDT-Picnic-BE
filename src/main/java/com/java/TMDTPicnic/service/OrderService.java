...
import com.java.TMDTPicnic.dto.response.OrderDetailResponse;
import com.java.TMDTPicnic.dto.response.OrderHistoryResponse;
import com.java.TMDTPicnic.dto.response.OrderItemDetailResponse;
...
import com.java.TMDTPicnic.dto.response.PaymentDetailResponse;
...
public class OrderService {
    ...
    /**
     * Admin xác nhận đã giao thành công với đơn COD (PAID -> COMPLETED)
     */
    @Transactional
    public OrderStatusUpdateResponse confirmDeliveryForCOD(Long orderId) {
        Order order = orderRepository.findById(orderId)
                .orElseThrow(() -> new RuntimeException("Order not found with id: " + orderId));
        ...
        if (order.getStatus() != OrderStatus.PAID) {
            throw new RuntimeException("Chỉ có thể xác nhận đơn COD đang ở trạng thái PAID");
        }

        if (payment.getPaymentMethod() == null || !payment.getPaymentMethod().equalsIgnoreCase(PaymentMethod.COD.name())) {
            throw new RuntimeException("Chỉ áp dụng xác nhận giao hàng cho đơn COD");
        }
        ...
        return OrderStatusUpdateResponse.builder()
                ...
                .message("Đã xác nhận giao hàng thành công cho đơn COD")
                .build();
    }

    /**
     * Lấy chi tiết đơn hàng cho user hiện tại
     */
    public OrderDetailResponse getOrderDetail(Long orderId, Long userId) {
        ...
        List<OrderItemDetailResponse> itemResponses = orderItems.stream()
                .map(item -> {
                    ...
                    return OrderItemDetailResponse.builder()
                            ...
                            .productThumbnail(product != null ? product.getThumbnail() : null)
                            .quantity(item.getQty())
                            .unitPrice(item.getUnitPrice())
                            .subtotal(item.getUnitPrice() != null && item.getQty() != null
        ...
        PaymentDetailResponse paymentResponse = paymentRepository.findByOrderId(orderId)
        ...
        return OrderDetailResponse.builder()
                ...
                .discountAmount(coupon != null ? coupon.getDiscountValue() : null)
                .sharedCartId(sharedCart != null ? sharedCart.getId() : null)
                .sharedCartTitle(sharedCart != null ? sharedCart.getTitle() : null)
                .createdAt(order.getCreatedAt())
    ...
    /**
     * User hủy đơn hàng khi còn ở trạng thái PENDING/PAID
     */
    @Transactional
    public OrderStatusUpdateResponse cancelOrder(Long orderId, Long userId) {
        Order order = orderRepository.findById(orderId)
                .orElseThrow(() -> new RuntimeException("Order not found with id: " + orderId));
        ...
        if (order.getStatus() != OrderStatus.PENDING && order.getStatus() != OrderStatus.PAID) {
            throw new RuntimeException("Chỉ có thể hủy đơn hàng ở trạng thái PENDING hoặc PAID");
        }
        ...
        for (OrderItem item : orderItems) {
            Product product = item.getProduct();
            if (product != null && item.getQty() != null) {
                product.setStockQuantity(product.getStockQuantity() + item.getQty());
                productRepository.save(product);
            }
        }
