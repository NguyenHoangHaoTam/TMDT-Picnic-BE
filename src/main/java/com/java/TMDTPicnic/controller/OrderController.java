...
public class OrderController {
    ...
    /**
     * Admin: Cập nhật trạng thái đơn hàng từ PAID sang SHIPPED hoặc COMPLETED
     * - SHIPPED: backward compatible với flow cũ
     * - COMPLETED: xác nhận giao hàng thành công (thường dùng cho COD)
     */
    @PutMapping("/{orderId}/status")
    @Operation(summary = "ROLE-ADMIN Cập nhật trạng thái đơn hàng (PAID -> SHIPPED/COMPLETED)")
    ...
    /**
     * Admin: Xác nhận đã giao hàng thành công cho đơn COD (PAID -> COMPLETED)
     * Endpoint riêng để rõ ràng hơn cho flow COD
     */
    @PutMapping("/{orderId}/confirm-delivery")
    @Operation(summary = "ROLE-ADMIN Xác nhận đã giao hàng cho đơn COD (PAID -> COMPLETED)")
    public ResponseEntity<ApiResponse<OrderStatusUpdateResponse>> confirmDelivery(
            @AuthenticationPrincipal Jwt jwt,
            @PathVariable Long orderId) {
        ...
        OrderStatusUpdateResponse response = orderService.confirmDeliveryForCOD(orderId);
     ...
     * - VNPay: Có thể xác nhận ngay khi PAID (đã thanh toán trước)
    ...
    @Operation(summary = "Xác nhận đã nhận hàng (PAID-VNPay hoặc SHIPPED -> COMPLETED)")
    ...
    /**
     * User: Hủy đơn hàng (chỉ cho phép khi status là PENDING hoặc PAID)
     */
    @PutMapping("/{orderId}/cancel")
    @Operation(summary = "Hủy đơn hàng (chỉ cho phép khi status là PENDING hoặc PAID)")
    public ResponseEntity<ApiResponse<OrderStatusUpdateResponse>> cancelOrder(
            @AuthenticationPrincipal Jwt jwt,
            @PathVariable Long orderId) {

        Long userId = Long.valueOf(jwt.getClaimAsString("sub"));
        OrderStatusUpdateResponse response = orderService.cancelOrder(orderId, userId);
